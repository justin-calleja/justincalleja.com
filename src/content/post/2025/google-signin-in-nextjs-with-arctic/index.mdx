...
1. The endpoint our button is linking to i.e. `/api/sign-in/google`. This will use `arctic` to create an authorization URL which we will redirect users to. In it, we'll add our intent to access the user's profile and email (technically, for this post I only need email but you may want e.g. profile pic). See https://developers.google.com/identity/protocols/oauth2/scopes for more on Google scopes.
   1. In this endpoint we'll also set secure HTTP-only cookies for the state and code verifier we used when generating the authorization URL. These will be used by our 2nd (callback) endpoint to verify the callback request made by Google to us.
2. After the user is redirected to Google, they will sign in there with one of their emails. Google will then redirect them to another endpoint on our Next.js server - the one we configured at the start in the Google console: `api/sign-in/google/callback`
