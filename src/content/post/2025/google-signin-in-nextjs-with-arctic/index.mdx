...
1. The endpoint our button is linking to i.e. `/api/sign-in/google`. This endpoint will use `arctic` to create an authorization URL which we will redirect users to. In the URL, we'll add our intent to access the user's profile and email (technically, for this post I only need email but you may want e.g. profile pic). See https://developers.google.com/identity/protocols/oauth2/scopes for more on Google scopes.
   - This endpoint will also take care of setting secure HTTP-only cookies for the state and code verifier used when generating the authorization URL. These will be used by our 2nd (callback) endpoint to verify the callback request made by Google to us.
2. After the user is redirected to Google, they will choose an account to sign in with and then Google will redirect them back to us - to another endpoint on our Next.js server - the one we configured at the start in the Google console: `api/sign-in/google/callback`
...
- we'll use the cookies we set before and some URL parameters Google adds on the request it makes to our callback endpoint - one of which is a code we need to validate.

This is step 1 of the OAuth flow:
User visits this endpoint → Gets redirected to Google
User authorizes on Google → Google redirects back with an authorization code
Your callback endpoint → Exchanges the code for an access token
Your app → Uses the token to get user info and create a session
The state and codeVerifier cookies ensure the callback is legitimate and secure.
