...
(i.e. it's on the `tweening` branch (end state of post). If you're going to clone, you'll get the `main` branch which is the starting point for this post).
...
What's important here is the Card's starting position. This needs to be the `Deck`'s `rect_global_position` because that's where I'd like the animation to start from, however, I'm adding the new Cards as children of `DealPosition`. This is because I want to group them under that parent node (I also know that I'll be using `DealPosition` exclusively to parent dealt cards so I can rely on it's children's size being the number of cards currently dealt and e.g. not have to stay storing that info myself in an array - and updating manually everytime this count changes).

Now, since I'm adding new cards as children of the `DealPosition`, their initial `global_rect_position` is going to be that of the `DealPosition`. This position is correct only after the cards have finished their "being dealt animation". However, initially, I need them to be at the deck's position so when I add a card to the node tree, I also change it's position to that of the deck's in `add_card_to_tree`.
...
After tweening (moving) the card, I'm waiting for the movement to finish before dealing another one (via `yield`), but you could use a timer instead and e.g. start dealing the next card before the previous one has finished moving to it's target position.
