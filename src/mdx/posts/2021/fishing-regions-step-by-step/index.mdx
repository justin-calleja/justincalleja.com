...
dateCreated: Jan 21, 2021 14:00:00 GMT+0000
tags: ['rpg maker', 'mz']
tocEnabled: true
excerpt: ''
---
...
1. Create a project e.g. "FishingEvent". 1. Add a map with some water in it and set
   the player's starting position to somewhere close to the water:
   <Image path="/add-fishing-village.png" alt="Add 'Fishing Village'" />
1. Pick and apply a `Region Id` (e.g. 1) to anywhere you want the player to be able
   to fish at. - Note how you need to be in "Map" mode and that you select the **R**
   tab.
   <Image path="/select-region-id.png" alt="Select region id" />
   <Image
     path="/setting-region-ids.png"
     alt="Setting the region id of applicable regions"
   />
...
1. From `Database ➜ Items`, (possibly change the maximum number of items), and add a "Fishing Rod" item, and (optionally) two fish items e.g. "Fish A" and "Fish B". I'm following the [original source material](https://www.xehryn.com/tutorials/fishing-regions) and choosing an item type of **Key Item** and setting the scope to **None** and occasion to **Always** for the "Fishing Rod".
   - Icons don't matter - they're just fillers.
     <Image
       path="/create-fishing-rod-and-fish.png"
       alt="Create the Fishing Rod"
     />
1. **Equip the Fishing Rod by adding an autorun event:** On scene start, in order to equip the player with the "Fishing Rod"; change to "Event mode" for the map, and place an event by double clicking on a tile which won't be interacted with (see e.g. in screenshot below).
   ...
   - Finally, set the new page's condition to self switch **A**. No contents are necessary.
     <Image
       path="/give-player-fishing-rod-on-start.png"
       alt="Give the player the fishing rod on start"
     />
     <Image
       path="/set-control-self-switch.png"
       alt="Set the control self switch A"
     />
...
<Image path="/add-script.png" alt="Add script to common event" />
...
<Image path="/truncated-script.png" alt="Truncated script" />
...
<Image
  path="/finishing-common-event-contents.png"
  alt="The rest of the common event's contents"
/>

To get the first check:

<Image path="/if-region-id-is-0.png" alt="Check if region id is 0" />
...
<Image path="/setting-the-fish-id.png" alt="Setting the fish id" />
...
<Image
  path="/final-try-fishing-contents.png"
  alt="Final 'Try Fishing' common event"
/>
...
<Image
  path="/using-try-fishing-common-event.png"
  alt="Using the 'Try Fishing' common event"
/>
...
<Image path="/fishing-common-event.png" alt="The 'Fishing' common event" />
...
<Image path="/fixed-code.png" alt="Fixed code in common events UI" />
...
<Image
  path="/add-myplugin.png"
  alt="Add the new plugin via the Plugin Manager"
/>

I went ahead and created a new Common Event to test it:

<Image
  path="/try-fishing-2.png"
  alt="The common event called 'Try Fishing 2'"
/>
