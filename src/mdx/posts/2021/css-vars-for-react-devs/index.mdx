...
dateCreated: Feb 09, 2021 14:00:00 GMT+0000
...
Problem `1` is the real issue to me. I haven't seen any issues with the site's performance on my machine but it is a relatively small site. It just doesn't feel right that so many re-renders have to be made every time the theme name changes.

## A better way

Anyway, I didn't revisit the issue until I read <a href="https://www.joshwcomeau.com/" target="_blank">Josh Comeau</a>'s awesome blog post: <a href="https://www.joshwcomeau.com/css/css-variables-for-react-devs/" target="_blank">CSS Variables for React Devs</a>. I had a feeling CSS variables would be "an alternative at least" - but I didn't want to lose focus and explore other options when the one I had was at least working.
...
… all using the approach outlined in Josh's post i.e. I should be able to just use a CSS variable to say what the color / width / font size is and they should change automatically based off of React state (the theme's state) and viewport size. Any changes to the DOM should only happen in one place, the definition (or re-definition) of the CSS variables. To do so, I'll be creating a simple sandbox working environment; the repo for which is: <a href="https://github.com/justin-calleja/css-vars-for-react-devs" target="_blank">css-vars-for-react-devs</a>
...
… in `theme.js` I'm saying I want a `font-size` of `1.6rem` as the "mobile" value (mobile first); and that any other `font-size` values after the first should only kick in at their corresponding breakpoints (`2rem` at `768px` or above etc…).
